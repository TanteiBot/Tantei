...
public sealed class SearchPickerTests
{
	...
	private static IEnumerable<string> EventNames(RecordingLogger<SearchPicker> logger) =>
		logger.Entries.Select(static entry => entry.EventId.Name ?? string.Empty);

	private static IEnumerable<string> Events(RecordingLogger<SearchPicker> logger, string eventName) =>
		logger.Entries
	...
	private static (Guid SearchId, PickerView View) Open(SearchPicker picker, FakePickerMessageTarget target, int resultCount = 2)
	{
