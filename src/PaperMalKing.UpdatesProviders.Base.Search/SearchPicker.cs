...
internal sealed class SearchPicker(IMemoryCache _cache, TimeProvider _timeProvider, ILogger<SearchPicker> _logger)
{
		...
		string providerDisplayName)
	{
		...
		return this.OpenCoreAsync(searchId, results, context, target, providerDisplayName);
	}
		...
		string providerDisplayName)
	{
		...
		var state = new PickerState(searchId, snapshot, context, target, providerDisplayName);
		try
		{
			this.Start(state);
...
#pragma warning restore CA1031
				...
				_cache.Set(SessionKey(state.SearchId), state, new MemoryCacheEntryOptions()
					...
					.RegisterPostEvictionCallback(
						static (_, value, _, owner) =>
						{
							...
							{
								((SearchPicker)owner!).OnEvicted(state);
							}
						},
						this));
			}
	...
	private async Task ChangePageAsync(PickerState state, IPickerInteraction interaction, int offset)
	{
		...
		lock (state.LifecycleGate)
		{
			...
			view = PickerRenderer.Render(state.Snapshot, state.SearchId, state.Page, state.ProviderDisplayName);
...
#pragma warning restore CA1031
	...
	private sealed record ExpiryTimerState(SearchPicker Picker, PickerState State);
